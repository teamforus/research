...
var compileConfig = function(configs, env) {
    ...
    for (var k in configs.platforms) {
        ...
        var old_libs_data = configs.platforms[k].libs_data || false;
        ...
        if (typeof old_libs_data != 'object')
            platforms[k].libs_data = sample_p.libs_data;
        else
            platforms[k].libs_data = deepExtend(platforms[k].libs_data || {}, sample_p.libs_data);
...
var qdt_core = {
    isReady: function(verbose) {
        if (!fs.existsSync('./qdt-env.json')) {
            if (verbose) console.log(
                colors.red(
                    "Please add \"qdt-env.json\" first.\n" +
                    "Run \"gulp init\" to create it, or use " +
                    "\"qdt-env-sample.json\"."
                ));
    ...
    ask: function(question) {
        return new Promise(function(resolve) {
            ...
            rl.on('line', function(line) {
                ...
                // ask again
                qdt_core.ask(qdt_verbose.core_ask_wrong_input)
                    .then(function(resp) {
                        resolve(resp);
                    });
            });
