    ...
    streams.push(gulp.src(src, {
        ...
        plugins.pug(Object.assign(pluginSettings.pug, {
            data: {
                ...
                env_data: qdt_e.data[platform.name]
            }
        })),
...
gulp.task('watch', ['server'], function () {
    ...
    var _iif_js = function (_k_js, platform, group) {
        ...
        _watch_src = group.src.concat(group.watch).map(function(_src) {
            return _path + _src;
        });

        _compile_src = group.src.map(function(_src) {
            return _path + _src;
        });

        gulp.watch(_watch_src, function (src) {
            js_compiler(platform, _compile_src, _settings);
        });
    };
